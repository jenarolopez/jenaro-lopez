...
import { ProjectArchive } from "@/types/archive";
import React, { useEffect, useState } from "react";

const Archive = () => {

  const [jsonData, setJsonData] = useState<ProjectArchive>([])
  ...
  const fetchJsonData = async () => {
    ...
    try {
      const response = await fetch(`/api/json?type=archieve`)
                ...
                <tr key={index} className="border-b border-slate-300/10 last:border-none">
                  ...
                  <td className="hidden py-4 pr-4 align-top lg:table-cell">
                    <ul className="flex -translate-y-1.5 flex-wrap">
                      {
                        project.builtWith.map((build, index) => (
                      ...
                      <li className="mb-1 flex items-center">
                        ...
                        >
                          <span>
                            {" "}
                            {
                              project.link && <span className="inline-block">
                                {project.link}
                                <svg
                                  xmlns="http:www.w3.org/2000/svg"
                                  viewBox="0 0 20 20"
